...
NP   = 300;%300;          % population size
runs = 25; %25;            % repetitions
global initial_flag; % the global flag used in test suite

FE= 1*10^4*D; %3*10^6; % budget of function evaluations

FUNCTIONS=0; %0 2 3 5 6 8 10 11 13 15 16 18 20]; % 0=sphere, the rest are the multimodal functions from CEC10 benchmark
...
if todo, disp('Running EDA-MCC...')
        ...
        opt2.c        = 20; %max group size (c in the paper). Note a typo on page 811 of the MCC paper, for their 500-dimensional experiments 
	  ...
	  % With these settings we have c x c = 20 x 20 covariance blocks to estimate from opt2.m = round(NP/2) points
        ...
        t2 = cputime;
        [x2, val2(run), stats2, opts2] = eda_mcc(@benchmark_func, opt2, pop, func_num, eps); % eps means to use the default shifts in the CEC2010 benchmark functions
	    % otherwise you can specify a shift (i.e. a number) for the last parameter 
        % print & save
        fprintf(1, 'EDA-MCC Elapsed CPU time: %f\n', cputime-t2);    
        fprintf(1, 'func_num = %d, run = %d\n', func_num, run);
