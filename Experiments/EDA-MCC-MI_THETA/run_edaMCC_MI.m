function run_edaMCC_MI(pD,pNp,pFun,pRuns,pTheta)
%addpath AMaLGaM
...
if todo, disp('Running EDA-MCC-MI...')
        ...
        opt2.ub       = ub;
        opt2.lb       = lb;
        opt2.theta    = pTheta;    % weakly dependent threshold (theta in the paper)
	  ...
	  % With these settings we have c x c = 20 x 20 covariance blocks to estimate from opt2.m = round(NP/2) points
        ...
        tic;
        [x2, val2(run), stats2, opts2 iter weak] = eda_mcc_mi(@benchmark_func, opt2, pop, func_num, eps); % eps means to use the default shifts in the CEC2010 benchmark functions
        ...
        fprintf(1, 'func_num = %d, run = %d, theta = %f\n', func_num, run, pTheta);
        fprintf(1, 'min(val) MCC=%f\n\n',val2(run));            
	%Time2=cputime-t2;
    	B(run,:) = stats2;
        save(['THETA_'  num2str(D) '_' num2str(NP) '_F' num2str(func_num) '_t' num2str(pTheta) '.mat'],'B');
        W(run,:) = weak;
        save(['WEAK_'  num2str(D) '_' num2str(NP) '_F' num2str(func_num) '_t' num2str(pTheta) '.mat'],'W');
        %FV(run,:) = stats2.fv;
